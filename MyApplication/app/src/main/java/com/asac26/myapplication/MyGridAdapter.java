...
public class MyGridAdapter extends ArrayAdapter {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        Date mounthDate = dates.get(position);
        Calendar dateCalendar = Calendar.getInstance();
        dateCalendar.setTime(mounthDate);
        ...
        TextView EventsNumber = view.findViewById(R.id.events_id);
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            if (DayNo == eventCalendar.get(Calendar.DAY_OF_MONTH) && displayMonth == eventCalendar.get(Calendar.MONTH) + 1 && displayYear == eventCalendar.get(Calendar.YEAR)) {
                ...
                EventsNumber.setText(arrayList.size() + "Events");
